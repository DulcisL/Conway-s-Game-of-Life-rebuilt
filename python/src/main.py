...
def drawGrid():
    ...
    for x in range(0, SCREEN.get_width(), gridOffset):
        # (display,color, start, end, thickness)
        pygame.draw.line(SCREEN, (0, 0, 60), (x, 0), (x, SCREEN.get_height()), 2)
    for y in range(0, SCREEN.get_height(), gridOffset):
        pygame.draw.line(SCREEN, (0, 0, 60), (0, y), (SCREEN.get_width(), y), 2)
...
def mainMenu():
    ...
    menu = pygame_menu.Menu(
        "Welcome to a python recreation of Conways's game of Life",
        MENU_X,
        MENU_Y,
        theme=mainTheme,
    )
...
def game():
    ...
    while RUNNING:
        ...
        # Snap to grid
        gx = (mx * SCALE_X) // CELL_SIZE
        gy = (my * SCALE_Y) // CELL_SIZE
...
def main():
    ...
    try:
        ...
        MENU_THEME.title_font_size = min(50, math.floor(100 * SCALE_X)) | max(
            50, math.floor(100 * SCALE_X)
        )
        MENU_THEME.widget_font_size = min(25, math.floor(50 * SCALE_X)) | max(
            25, math.floor(100 * SCALE_X)
        )
